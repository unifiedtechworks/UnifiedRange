...
Steps 1-7 are implemented through detail-only backend delivery/rendering. Phase 2F.1 adds the owner-authorized cleanup primitive with public-prefix delete permission, and Phase 2F.2 wires its snapshot-id-only Public Preview removal control. Derivative-aware unpublish, replacement, target-photo support, moderation actions, and lifecycle reconciliation remain unavailable. The derivative namespace is not directly public; only the resolver Lambda can issue a short-lived eligible-object URL.
