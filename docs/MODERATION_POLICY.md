...
- Phase 2G.2 initially created snapshot-bound reports without exposing an asset id. Phase 2G.5 replaces that product path with the authenticated `createPublicImageReport` action. The browser supplies only the public snapshot id, allow-listed reason, and bounded optional details; the backend derives reporter identity and the exact eligible image generation. Report submission never automatically hides or removes an image.
...
- New Phase 2G.5 image reports are generation-bound; older rows without `publicImageAssetId` remain clearly labeled legacy/unbound. The Phase 2G.4 control remains current-snapshot scoped and does not automatically act on the bound historical generation. Reviewers must freshly inspect the linked public setup before confirming. Exact-generation moderator preview/action, cross-generation holds, notifications, and append-only audit remain planned in the [Phase 2G Public Image Moderation Plan](PUBLIC_IMAGE_PHASE_2G_MODERATION_PLAN.md).
- `createPublicImageReport` requires Cognito user-pool authentication, validates exact `ready + clear + equipment_cover` eligibility and current public visibility, then transactionally creates an `open` report and updates `lastReportAt`. It accepts no reporter id, asset id, key, path, URL, owner/source id, filename, target-photo data, or image bytes. Raw and normalized details are both limited to 500 characters. Before invocation, the detail UI re-resolves delivery and refuses submission if the rendered generation is no longer current; the backend transaction then rejects changes during validation/commit.
...
- New public-image reports show **Generation-bound report** with only a shortened technical generation reference to authorized reviewers. Existing rows without a binding show **Legacy/unbound report** and remain reviewable/status-editable. A malformed non-empty binding is labeled **Unrecognized generation binding**, its value is not displayed, and it is never trusted as proof of a generation.
