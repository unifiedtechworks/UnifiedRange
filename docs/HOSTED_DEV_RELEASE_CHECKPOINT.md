...
13. Sign in as an `admin` or `moderator`. Confirm Moderation navigation appears, `/moderation/reports` loads metadata only, pending counts are correct, and all supported report statuses persist after refresh. For a `public_image` report, confirm the card is clearly labeled, its public setup link opens only the sanitized public detail in a new tab, no image is embedded/resolved in moderation, and status changes do not affect public image availability.
...
- Public image rendering is limited to eligible processed Equipment Passport covers on saved Public Passport detail. Uploaded originals remain private, and Discover/profile cards and target photos remain image-free. Phase 2F.1 provides the backend removal primitive, Phase 2F.2 adds its owner-only Public Preview remove control, Phase 2F.3 detaches an attached derivative before owner unpublish continues, and Phase 2F.4 adds an owner-facing remove-first replacement flow. Phase 2G.1 adds moderation/report schema guardrails and blocked-state enforcement. Phase 2G.2 adds a signed-in **Report image** flow only beside a successfully loaded detail image. Phase 2G.3 makes those reports distinct in moderation and links to the existing sanitized public detail without embedding a preview. Replacement remains non-atomic, and account/private-source hooks, durable cleanup reconciliation, trusted image-report generation binding, exact-generation moderator preview, and moderator image actions remain unavailable. See the [Public Image Publishing Plan](PUBLIC_IMAGE_PUBLISHING_PLAN.md).
- Feeds and follows are not implemented.
- A notification center is not implemented.
- Moderation has report metadata review and status workflow only. Phase 2G.3 public-image cards add a sanitized public-detail link, but reports remain snapshot-bound/generation-unbound and there is no embedded image preview or image action. There is still no destructive content removal, automatic image hiding, moderator image removal, suspension, or account action. See the [Phase 2G Public Image Moderation Plan](PUBLIC_IMAGE_PHASE_2G_MODERATION_PLAN.md).
...
4. Continue the [Phase 2G Public Image Moderation Plan](PUBLIC_IMAGE_PHASE_2G_MODERATION_PLAN.md) by replacing the Phase 2G.2 interim direct report with a trusted exact-generation binding command, then add a purpose-built exact-generation review projection and Phase 2G.4 audited moderator hide/remove before considering Discover images; keep report status independent from image availability.
...
The Phase 2G.3 increment makes snapshot-level public-image reports understandable in the existing group-gated moderation queue and links only to sanitized public detail. It adds no embedded moderation image, ledger access, backend/schema change, or moderator image action and does not widen the product boundary. UnifiedRange remains a privacy-first recordkeeping, readiness, and sanitized setup-discovery product. It does not provide ballistic calculators, scope outputs, hold recommendations, field corrections, sight-in instructions, or instructions for adjusting or aiming equipment.
