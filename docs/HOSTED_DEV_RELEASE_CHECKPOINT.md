...
- Public image rendering is limited to eligible processed Equipment Passport covers on saved Public Passport detail. Uploaded originals remain private, and Discover/profile cards and target photos remain image-free. Phase 2F.1 provides the backend removal primitive, Phase 2F.2 adds its owner-only Public Preview remove control, Phase 2F.3 detaches an attached derivative before owner unpublish continues, and Phase 2F.4 adds an owner-facing remove-first replacement flow. Phase 2G.1 adds moderation/report schema guardrails and blocked-state enforcement. Phase 2G.2 adds a signed-in **Report image** flow only beside a successfully loaded detail image. Replacement remains non-atomic, and account/private-source hooks, durable cleanup reconciliation, trusted image-report generation binding, safe moderator preview, and moderator image actions remain unavailable. See the [Public Image Publishing Plan](PUBLIC_IMAGE_PUBLISHING_PLAN.md).
- Feeds and follows are not implemented.
- A notification center is not implemented.
- Moderation has report metadata review and status workflow only. Phase 2G.2 public-image reports can enter that metadata queue, but they are snapshot-bound only and do not add an image preview or image action. There is still no destructive content removal, automatic image hiding, moderator image removal, suspension, or account action. See the [Phase 2G Public Image Moderation Plan](PUBLIC_IMAGE_PHASE_2G_MODERATION_PLAN.md).
...
4. Continue the [Phase 2G Public Image Moderation Plan](PUBLIC_IMAGE_PHASE_2G_MODERATION_PLAN.md) by replacing the Phase 2G.2 interim direct report with a trusted exact-generation binding command, then add the Phase 2G.3 safe review projection and Phase 2G.4 audited moderator hide/remove before considering Discover images; keep report status independent from image availability.
