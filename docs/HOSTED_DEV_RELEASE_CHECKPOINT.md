...
- Distinct metadata-only public-image report cards with a link to the same sanitized Public Passport detail route available to visitors; moderation does not embed or resolve images.
- Backend-controlled public-image reporting binds each new report to the exact eligible derivative generation without accepting reporter, asset, owner/source, key/path, URL, filename, or image-byte input from the browser. Legacy unbound reports remain supported.
- Report workflow statuses: `open`, `reviewed`, `dismissed`, and `action_needed`.
- Moderators can update report status; status changes do not delete, hide, suspend, or mutate reported content.
- A separate Phase 2G.4 admin/moderator action can hide or remove only the currently attached processed public derivative. It revalidates the backend-derived asset/key, detaches delivery first, preserves sanitized public text/setup and the private original, and never changes report status.
...
9. Preview and publish a sanitized Equipment Passport snapshot. Confirm **Publish without images** is the default. With a disposable verified JPEG/PNG equipment cover, test the optional consent flow and confirm only saved Public Passport detail renders the eligible processed derivative. Confirm parent publish/remove/replace/unpublish controls stay disabled for the full processor request. Test owner removal while retaining the text snapshot; then prepare a disposable derivative, upload and verify a new cover, and test remove-first replacement. Confirm the old derivative is detached before the replacement consent flow begins and a snapshot-save or processing failure leaves the sanitized setup public without an image and offers a safe retry. Finally, prepare a derivative again and test derivative-aware Unpublish. Confirm public image delivery detaches before the text/setup disappears, the private original stays unchanged, and throttled route/account/private-cover changes never apply a late cleanup/unpublish/replacement response to another preview or source.
...
13. Sign in as an `admin` or `moderator`. Confirm Moderation navigation appears, `/moderation/reports` loads metadata only, pending counts are correct, and all supported report statuses persist after refresh. For a disposable `public_image` report, confirm the card is clearly labeled, its public setup link opens only sanitized public detail in a new tab, and no image is embedded/resolved in moderation. Inspect the current public setup immediately before testing Hide and Remove. Confirm delivery detaches, only the processed public derivative is affected, the private original/text snapshot remain, bounded statuses appear, and `Report.status` is unchanged.
14. Sign in as a normal user and confirm Moderation navigation is hidden, moderation routes expose no report data/status/action controls, and direct image-moderation mutation invocation is unauthorized.
...
- Public image rendering is limited to eligible processed Equipment Passport covers on saved Public Passport detail. Uploaded originals remain private, and Discover/profile cards and target photos remain image-free. Phase 2F provides owner removal, derivative-aware Unpublish, and remove-first replacement. Phase 2G.1-2G.4 add moderation/report guardrails, signed-in image reporting, metadata-only review, and current-snapshot moderator/admin hide/remove. Phase 2G.5 binds new public-image reports to the exact generation current at submission. Replacement remains non-atomic, and account/private-source hooks, durable cleanup reconciliation, exact-generation moderator preview/action, cross-generation moderation holds, audit, notifications, and broader image rendering remain unavailable. See the [Public Image Publishing Plan](PUBLIC_IMAGE_PUBLISHING_PLAN.md).
- Feeds and follows are not implemented.
- A notification center is not implemented.
- Moderation has metadata review, report status, generation-bound new image reports, and a separate current-public-derivative hide/remove action. Legacy reports may remain unbound, the queue has no embedded image preview, and reviewers must freshly inspect the current sanitized public route before an action because the action is not yet report-generation scoped. Hide retains its processed derivative; if the snapshot is later unpublished before moderator Remove, delivery stays unavailable but protected orphan reconciliation is needed because the current action will not guess a detached generation. There is still no destructive public-text/private-content removal, automatic hiding, suspension, account action, exact-generation action, durable moderation hold, or append-only audit. See the [Phase 2G Public Image Moderation Plan](PUBLIC_IMAGE_PHASE_2G_MODERATION_PLAN.md).
...
4. Continue the [Phase 2G Public Image Moderation Plan](PUBLIC_IMAGE_PHASE_2G_MODERATION_PLAN.md) with a purpose-built exact-generation review projection/action, durable hold, audit, and reconciliation before considering Discover images; keep report status independent from image availability.
...
The Phase 2G.4 increment adds a narrowly authorized current-public-derivative hide/remove action without embedding an image, widening client ledger access, touching private originals, or changing sanitized public text/setup. It requires backend deployment and hosted authorization/lifecycle testing. UnifiedRange remains a privacy-first recordkeeping, readiness, and sanitized setup-discovery product. It does not provide ballistic calculators, scope outputs, hold recommendations, field corrections, sight-in instructions, or instructions for adjusting or aiming equipment.
