...
- Public image rendering is limited to eligible processed Equipment Passport covers on saved Public Passport detail. Uploaded originals remain private, and Discover/profile cards and target photos remain image-free. Phase 2F provides owner removal, derivative-aware Unpublish, and remove-first replacement. Phase 2G.1-2G.3 add moderation/report guardrails, signed-in image reporting, and metadata-only review. Phase 2G.4 adds current-snapshot moderator/admin hide/remove. Replacement remains non-atomic, and account/private-source hooks, durable cleanup reconciliation, trusted image-report generation binding, exact-generation moderator preview, cross-generation moderation holds, audit, notifications, and broader image rendering remain unavailable. See the [Public Image Publishing Plan](PUBLIC_IMAGE_PUBLISHING_PLAN.md).
- Feeds and follows are not implemented.
- A notification center is not implemented.
- Moderation has metadata review, report status, and a separate current-public-derivative hide/remove action. Reports remain snapshot-bound/generation-unbound, the queue has no embedded image preview, and reviewers must freshly inspect the current sanitized public route before an action. Hide retains its processed derivative; if the snapshot is later unpublished before moderator Remove, delivery stays unavailable but protected orphan reconciliation is needed because the current action will not guess a detached generation. There is still no destructive public-text/private-content removal, automatic hiding, suspension, account action, durable moderation hold, or append-only audit. See the [Phase 2G Public Image Moderation Plan](PUBLIC_IMAGE_PHASE_2G_MODERATION_PLAN.md).
...
4. Continue the [Phase 2G Public Image Moderation Plan](PUBLIC_IMAGE_PHASE_2G_MODERATION_PLAN.md) by replacing the Phase 2G.2 interim direct report with a trusted exact-generation binding command, then replace the current-snapshot convention with a purpose-built exact-generation review projection, durable hold, audit, and reconciliation before considering Discover images; keep report status independent from image availability.
