...
9. Preview and publish a sanitized Equipment Passport snapshot. Confirm **Publish without images** is the default. With a disposable verified JPEG/PNG equipment cover, test the optional consent flow and confirm only saved Public Passport detail renders the eligible processed derivative. Confirm parent publish/remove/unpublish controls stay disabled for the full processor request. From Public Preview, remove the derivative and confirm the text/setup snapshot stays published, public detail becomes image-free, and the private original is unchanged. Under network throttling, change routes during cleanup and confirm the late result does not overwrite the new preview. Use the resulting text-only snapshot for the current unpublish test.
...
- Public image rendering is limited to eligible processed Equipment Passport covers on saved Public Passport detail. Uploaded originals remain private, and Discover/profile cards and target photos remain image-free. Phase 2F.1 provides the backend removal primitive and Phase 2F.2 adds its owner-only Public Preview remove control. Replacement, derivative-aware unpublish, account/private-source hooks, and image moderation remain unavailable. See the [Public Image Publishing Plan](PUBLIC_IMAGE_PUBLISHING_PLAN.md).
...
3. Complete hosted Phase 2C-2E image processing/delivery tests plus Phase 2F.1/2F.2 cleanup checks, then add derivative-aware unpublish, replacement generations, and visibility/private-source cleanup before considering wider rendering.
