...
9. Preview and publish a sanitized Equipment Passport snapshot. Confirm **Publish without images** is the default. With a disposable verified JPEG/PNG equipment cover, test the optional consent flow and confirm only saved Public Passport detail renders the eligible processed derivative. From Public Preview, remove the derivative and confirm the text/setup snapshot stays published, public detail becomes image-free, and the private original is unchanged. Use the resulting text-only snapshot for the current unpublish test.
