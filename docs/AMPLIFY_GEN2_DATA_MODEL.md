...
The `public_image` enum value and protected generation field are contract reservations only. An unbound direct-model row must remain non-actionable, and existing moderation status updates must remain independent from image availability.
