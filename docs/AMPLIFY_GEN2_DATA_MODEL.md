...
Phase 2G.1 extends `ReportTargetType` with `public_image` and adds optional `Report.publicImageAssetId` as the protected immutable-generation binding. Field authorization permits Cognito `admin`/`moderator` reads plus report-owner read/delete for generated-operation compatibility, while denying reporter create/update and all public/API-key access. No current UI displays or accepts the value. The existing generic model-create path therefore cannot create an actionable public-image report. Phase 2G.2 must add a signed-in snapshot-id-only backend command that validates current eligibility and writes the binding itself. No public-image report button, moderator review projection/UI, moderator image action, audit model, or new report index exists yet.
