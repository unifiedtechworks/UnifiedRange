...
## Phase 1 through Phase 2G.1 Public Image Foundation

`PublicImageAsset` is a non-public workflow ledger for the backend image processor. It contains the public snapshot relationship, source type and record id, processed public derivative fields, bounded processing status/error data, consent timestamp, and a separate moderation state. It deliberately contains no private S3 key and has no API-key authorization.

Current owner clients may read their own workflow records but cannot create, update, or delete them. `processPublicPassportImage` can create/update processing state, and `removePublicPassportImage` can conditionally mark the current asset removed and clear its public cleanup fields. Phase 2G.1 adds `PublicImageModerationStatus` (`clear`, `hidden`, `removed`) plus owner-readable/client-nonwritable `moderationStatus`, `hiddenAt`, `removedAt`, `moderationReason`, and `lastReportAt`. Actor fields are intentionally omitted pending a protected audit model. Admin/moderator access to the full ledger remains unavailable; the Phase 2G.3 link-only review UI does not read it, and any future exact-generation preview/action must use a limited backend projection.
...
The derivative Storage namespace is `public/passports/{snapshot_id}/cover/*`. The processor has private-equipment get plus derivative get/write/delete access. The Phase 2E.1 resolver has derivative get access only; the Phase 2F.1 cleanup and Phase 2G.4 moderation Lambdas have derivative delete only. No browser, guest, API-key, moderator, or admin identity can access the prefix directly. Phase 2D may call the processor after explicit consent, while `buildPublicPassportSnapshotInput` still omits image fields.

`resolvePublicPassportImage` is an API-key-authorized query with one input: `publicPassportSnapshotId`. Its Lambda re-reads attribute-limited public snapshot, public asset, Equipment Passport public flag, and profile visibility data, then checks the exact derivative with S3 `HeadObject`. The owner index locates the profile, and a consistent primary-key read confirms current visibility before signing. It never reads `PrivateImageAsset`, a private image key, target-photo data, or a private Storage prefix. An eligible `ready` `equipment_cover` with `moderationStatus = clear` returns a 60-second signed URL, safe alt text, expiry, and zero cache seconds. Phase 2G.1 temporarily also accepts a missing moderation value for legacy rows; hidden, removed, unknown, mismatched, private, failed, or unavailable cases share the same generic unavailable response. Saved Public Passport detail may render that derivative; Discover and public profiles remain image-free.

Phase 2G.5 removes API-key model-read authorization from `PublicPassportSnapshot.coverPhotoUrl`, `publicImageAssetId`, `publicImageKey`, and `publicImageAltText`. Discover, public profile, and public detail snapshot queries use one explicit sanitized selection that omits those fields. The public delivery resolver is therefore the only path that returns image delivery data, and it returns no raw key or generation id.

Phase 2F.1 adds `PublicImageCleanupStatus`, the `removePublicPassportImage(publicPassportSnapshotId)` owner mutation, and the `publicImageAssetsBySnapshotId` secondary index. The cleanup Lambda revalidates ownership, conditionally removes the snapshot's three image projection fields and marks a safely bound `equipment_cover` ledger row `removed` before S3 deletion. A removed row retains its canonical public key only while deletion needs a retry; a GSI lookup followed by consistent primary-key reads lets repeated snapshot-id-only calls retry safely. Successful cleanup removes the ledger key and alt-text copy. The processor now includes snapshot `updatedAt` in finalization conditions and cannot reactivate a removed asset id, preventing an older processing attempt from winning after cleanup. Phase 2F.2 calls this mutation from an owner-only Public Preview remove control, Phase 2F.3 calls it before the existing owner-scoped snapshot deletion for derivative-aware Unpublish, and Phase 2F.4 reuses it for remove-first replacement. Visibility/private-source hooks, target-photo support, and scheduled reconciliation remain future work.

Phase 2G.1 extends `ReportTargetType` with `public_image` and adds optional `Report.publicImageAssetId` as the protected immutable-generation binding. Field authorization permits Cognito `admin`/`moderator` reads plus report-owner read/delete for generated-operation compatibility, while denying reporter create/update and all public/API-key access. Phase 2G.5 adds `createPublicImageReport(publicPassportSnapshotId, reason, details?)`: a user-pool-authenticated Lambda derives the reporter and exact current asset, validates the public snapshot/source/profile graph, rejects raw or normalized details above 500 characters, and transactionally creates the `open` bound report while updating `PublicImageAsset.lastReportAt`. The response returns only a bounded submission status/failure code. Existing unbound rows remain compatible, while malformed non-empty bindings are treated as unrecognized rather than generation-bound. Phase 2G.3 provides metadata-only review with a link to the current sanitized public setup. Phase 2G.4 adds `PublicImageModerationAction`, `PublicImageModerationActionStatus`, and `moderatePublicPassportImage(publicPassportSnapshotId, action, reason?)`, authorized only to Cognito `admin`/`moderator` groups. The function repeats group validation, derives every asset/key/source/owner value, conditionally detaches only a current canonical `ready + clear + equipment_cover` projection, and never reads a private table or prefix. Hide marks the current asset hidden; Remove marks it removed and deletes only the public derivative. Both leave report status, public text/setup, and the private original unchanged.

New or safely reprocessed public-image ledger rows initialize `moderationStatus` to `clear`. Processor conditions reject a row with any other non-empty moderation state, preventing hidden/removed generations from being restored through reprocessing. Phase 2G.4 requires exact `clear` and fails closed for a legacy missing value; eligible legacy rows therefore need controlled backfill/reprocess before the action can moderate them. The resolver's temporary missing-state compatibility remains until that migration is complete.
...
The `public_image` generation field is written only by `createPublicImageReport`; the reporter cannot provide it. New reports are bound to the exact eligible generation current when the transaction commits, while pre-2G.5 rows with no binding remain legacy/unbound. Phase 2G.4 does not consume that binding: it explicitly acts on the current snapshot projection after fresh reviewer inspection and backend conditional validation. Existing report-status updates remain independent from image availability. Exact reported-generation preview/action, durable cross-generation holds, audit, and notification remain future work.
