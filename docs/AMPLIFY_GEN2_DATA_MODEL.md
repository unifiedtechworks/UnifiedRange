...
Current owner clients may read their own workflow records but cannot create, update, or delete them. `processPublicPassportImage` can create/update processing state, and `removePublicPassportImage` can conditionally mark the current asset removed and clear its public cleanup fields. Phase 2G.1 adds `PublicImageModerationStatus` (`clear`, `hidden`, `removed`) plus owner-readable/client-nonwritable `moderationStatus`, `hiddenAt`, `removedAt`, `moderationReason`, and `lastReportAt`. Actor fields are intentionally omitted pending a protected audit model. Admin/moderator access to the full ledger remains unavailable; Phase 2G.3 must use a limited backend projection.
