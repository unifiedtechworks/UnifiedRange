...
The `public_image` generation field remains a contract reservation until a trusted report command can write it. Phase 2G.4 does not infer that binding: it explicitly acts on the current snapshot projection after fresh reviewer inspection and backend conditional validation. Existing report-status updates remain independent from image availability. Exact reported-generation targeting, durable cross-generation holds, audit, and notification remain future work.
