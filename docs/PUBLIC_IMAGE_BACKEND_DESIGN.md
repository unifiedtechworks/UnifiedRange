...
- `ReportTargetType` includes `public_image`. `Report.publicImageAssetId` cannot be supplied or updated by a reporter or accessed publicly; report owners retain read/delete authorization for generated-operation compatibility, and `admin`/`moderator` groups may read it. Phase 2G.5 routes the detail-only report form through a trusted backend action that derives and writes this exact-generation binding.
...
- `PublicPassportSnapshot.publicImageAssetId`, `publicImageKey`, and `publicImageAltText` are owner-readable and client-nonwritable. Public model operations use an explicit sanitized selection and cannot read them; public image delivery remains available only through the guarded snapshot-id resolver.
...
- API-key model readers cannot read the projected derivative key, asset id, or alt-text field. Public detail receives only a short-lived derivative URL and safe alt text through the guarded resolver.
...
Phase 2G.1 reserves the dedicated `public_image` report target whose client-visible `targetId` is the public snapshot id. The protected `Report.publicImageAssetId` field binds the report to the immutable `PublicImageAsset.id` generation current at submission. The reporter cannot populate or update that field; minimum owner read/delete authorization preserves existing generated model-operation compatibility, and no public product UI renders it. Phase 2G.5 replaces the Phase 2G.2 interim direct model create with `createPublicImageReport`, which accepts only the snapshot id, allow-listed reason, and bounded optional details. Its Lambda derives reporter identity and the current asset, validates exact public eligibility, and conditionally writes the binding.

Phase 2G.4 remains a current-snapshot action even after report binding. Its UI requires a fresh review of the linked current public setup and calls the action with only the snapshot id, action, and bounded reason. The backend re-reads the currently attached projection/asset, derives the canonical path, and conditionally detaches it. A concurrent projection change fails closed. A generation-bound report can still be presented beside a later replacement, so an exact-generation review/action and durable cross-generation hold remain required before broader image rendering.
...
Phase 2G.4 implements separate authorization, detach-first revocation, private-source preservation, and report-status independence. It writes only the existing bounded owner-readable moderation reason/timestamps on the ledger. Phase 2G.5 implements trusted report-generation binding and `lastReportAt` updates with attribute-limited transactional IAM and no Storage permission. Lambda logs contain fixed event names, action/status, and bounded failure codes only; they omit ids, keys, URLs, filenames, alt text, reason/details content, report/profile data, credentials, and raw AWS errors. Append-only actor/action audit, exact-generation moderation actions, durable moderation holds, and notification/appeal policy remain future work.
...
- [x] Route derivative-aware Unpublish through backend cleanup before owner-scoped snapshot deletion.
