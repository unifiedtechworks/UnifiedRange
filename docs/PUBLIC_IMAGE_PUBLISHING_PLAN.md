...
- Phase 2G.1 reserves `public_image` with the safe public snapshot id as `targetId` and a separate reporter-nonwritable immutable asset-generation binding. Phase 2G.2 provides the first supported detail-only report form but leaves that protected field unset. A future backend report command remains required before reports can safely drive image actions.
...
- Phase 2G.4 provides a separately group-authorized current-derivative hide/remove action without deleting the owner's private image. Append-only audit and exact reported-generation targeting remain future hardening.
...
Public-image reporting, metadata review, and current-derivative hide/remove now exist only around saved Public Passport detail and the group-gated moderation queue. The report Lambda binds new reports to the exact current eligible generation without accepting internal identifiers, and the moderator Lambda has no private-table or private-prefix permission. The moderation UI never embeds an image or exposes ledger/storage identifiers beyond a shortened generation reference. Legacy reports remain unbound, and the action remains current-snapshot scoped, so reviewers must inspect the current public route before acting. An exact-generation review/action, cross-generation hold, audit, and hosted adversarial testing remain required before public image rendering expands beyond saved Public Passport detail.
...
- Hosted same-owner success and adversarial integration fixtures are still required. Phase 2F.1 provides explicit backend removal for the current derivative, Phase 2F.2 provides its owner-only Public Preview control, and Phase 2F.3 composes that cleanup with owner-scoped snapshot deletion for derivative-aware unpublish; superseded-derivative cleanup, replacement, and scheduled orphan reconciliation remain future lifecycle work.
...
- Phase 2F.2 implements owner removal, Phase 2F.3 implements derivative-aware Unpublish, and Phase 2F.4 implements owner-facing remove-first replacement. All detach delivery before the later lifecycle step; atomic replacement remains unimplemented.
...
- Implemented `removePublicPassportImage(publicPassportSnapshotId)` with user-pool authorization and no key/URL/owner/source/destination input.
- The operation conditionally removes all guarded snapshot image fields and marks a safely matched `equipment_cover` ledger row `removed` before S3 deletion.
- A snapshot-id ledger index lets repeated calls retry a canonical key retained only while cleanup is pending; successful deletion clears the ledger key and alt-text copy.
...
- Phase 2F.2 adds the owner-facing remove control, Phase 2F.3 adds derivative-aware Unpublish, and Phase 2F.4 adds remove-first replacement using a newly verified immutable private candidate/public derivative generation. Account/private-source hooks and scheduled reconciliation remain later work; current-derivative moderation is implemented separately in Phase 2G.4.
...
- Phase 2G.2 detail-only report UI now invokes the Phase 2G.5 backend action with only snapshot id, reason, and optional details. The Lambda derives reporter identity/current generation and transactionally creates the bound report plus `lastReportAt`. Phase 2G.3 adds metadata-only moderator cards and a sanitized public-detail link without resolving an image or reading a ledger. Phase 2G.4 adds a separate admin/moderator mutation whose browser input is limited to snapshot id, `hide | remove`, and optional bounded reason; the Lambda conditionally derives/detaches only the current canonical `equipment_cover` derivative and preserves private originals/public text.
- Exact-generation moderator projection/action, a cross-generation moderation hold, append-only audit, notification/appeal policy, and durable reconciliation remain unavailable. Legacy unbound reports stay reviewable/status-editable, and legacy assets without exact `moderationStatus = clear` fail closed for Phase 2G.4 until controlled backfill/reprocess.
