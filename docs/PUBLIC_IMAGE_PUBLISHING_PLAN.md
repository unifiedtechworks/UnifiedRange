...
Public-image reporting, metadata review, and current-derivative hide/remove now exist only around saved Public Passport detail and the group-gated moderation queue. The moderator Lambda has no private-table or private-prefix permission, and the UI never embeds an image or exposes ledger/storage identifiers. Reports remain generation-unbound; reviewers must inspect the current public route before acting. Trusted binding, an exact-generation review projection, cross-generation hold, audit, and hosted adversarial testing remain required before public image rendering expands beyond saved Public Passport detail.
...
- Trusted report-generation binding, exact-generation moderator projection/preview, a cross-generation moderation hold, append-only audit, notification/appeal policy, and durable reconciliation remain unavailable. Legacy assets without exact `moderationStatus = clear` fail closed for Phase 2G.4 until controlled backfill/reprocess.
