...
- Hosted same-owner success and adversarial integration fixtures are still required. Phase 2F.1 now provides explicit backend removal for the current derivative and Phase 2F.2 provides its owner-only Public Preview control; superseded-derivative cleanup, derivative-aware unpublish, replacement, and scheduled orphan reconciliation remain future lifecycle work.
...
- Direct Unpublish and replacement are disabled after a derivative is prepared. Phase 2F.1 implements backend-controlled removal as a developer-tested primitive; owner remove/replace UI and derivative-aware unpublish remain unimplemented.
...
- Phase 2F must add backend-controlled removal, derivative-aware unpublish, replacement, visibility/private-source revocation, reconciliation, and audited image moderation before any broader rendering surface. Follow [PUBLIC_IMAGE_PHASE_2F_LIFECYCLE_CLEANUP_PLAN.md](PUBLIC_IMAGE_PHASE_2F_LIFECYCLE_CLEANUP_PLAN.md).
...
- Phase 2F.2 adds the owner-facing remove control. A later phase must add immutable replacement generations and derivative-aware unpublish. Account/private-source hooks, scheduled reconciliation, and image moderation remain later work.
