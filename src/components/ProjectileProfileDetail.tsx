  ...
  if (state === "missing") {
    return (
      <section className="rounded-md border border-ink/10 bg-white p-5 shadow-soft">
        ...
        <p className="mt-2 text-sm leading-6 text-ink/70">{error || "This Projectile / Ammo profile is not available for the current account or sample data set."}</p>
...
function ProjectileDetailContent({ projectile, source }: { projectile: ProjectileProfile; source: "saved" | "demo" }) {
  return (
    <section>
      <PageHeader
        eyebrow={source === "saved" ? "Saved private Projectile / Ammo" : "Sample Projectile / Ammo"}
