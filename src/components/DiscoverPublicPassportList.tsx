    ...
    <div className="space-y-6">
      ...
      <section className="rounded-md border border-ink/10 bg-white p-5 shadow-soft">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
          ...
          {activeFilters ? (
            <button type="button" onClick={() => setFilters(emptyFilters)} className="w-full rounded-md border border-ink/15 bg-white px-4 py-2 text-sm font-semibold text-ink sm:w-fit">
              Reset filters
            </button>
          ) : null}
        </div>
      ...
      {state === "ready" && filteredSnapshots.length === 0 ? (
        <section className="rounded-md border border-ink/10 bg-white p-5 text-center shadow-soft">
          ...
          <button type="button" onClick={() => setFilters(emptyFilters)} className="mt-4 w-full rounded-md bg-ink px-4 py-2 text-sm font-semibold text-white sm:w-auto">
            Reset filters
          </button>
        </section>
      ) : null}
  ...
  return (
    <label className="block">
      ...
      <input
        ...
        className="mt-2 min-h-10 w-full rounded-md border border-ink/15 bg-white px-3 py-2 text-sm text-ink outline-none focus:border-moss"
      />
    </label>
  );
}
