...
- Phase 2G public-image moderation/report guardrails, detail-only signed-in image reporting, metadata-only moderator review, and a separate admin/moderator current-image hide/remove action with detach-first delivery revocation
...
Phase 2G.2 reports are still generation-unbound, so the current Phase 2G.4 control is intentionally a current-snapshot action rather than proof that a reported generation is still attached. Reviewers must open and inspect the linked public setup immediately before confirming. Conditional writes reject changed projections, but trusted report-generation binding, exact-generation moderator preview, a durable cross-generation moderation hold, audit trail, notifications, and lifecycle reconciliation remain unimplemented. Legacy eligible asset rows without `moderationStatus = clear` fail closed for the action until a controlled backfill/reprocess. Moderators have no broad image-ledger, private-record, or private-original access. See [Phase 2G Public Image Moderation Plan](docs/PUBLIC_IMAGE_PHASE_2G_MODERATION_PLAN.md).
...
8. Confirm the public-image report has a clear **Public image** target label, muted public snapshot ID, reason/details, safe reporter identity, timestamps, and current status.
...
The live backend boundary covers Cognito auth, owner-scoped records, private S3 images, owner-only private image candidates with trusted source verification, sanitized public snapshots, public identity snapshots, social actions, detail-only processed public-image delivery, owner image lifecycle controls, and group-gated report/public-image moderation. Broader public-image surfaces, durable moderation audit/holds, account lifecycle execution, and secure username sign-in remain planned work.
