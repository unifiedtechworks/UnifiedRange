...
The hardened Phase 2F.2-2F.4 UI blocks publish, processing, removal, replacement, and unpublish operations from overlapping. Cleanup, unpublish, and replacement responses are scoped to the route and signed-in owner context, so a late response cannot replace a newer Public Preview state. Image-bearing Unpublish and remove-first replacement continue only after cleanup returns `removed`, `not_attached`, or detach-confirmed `cleanup_pending`; a failure leaves the text snapshot published and does not start the next step. If replacement processing fails after detachment, the setup remains safely text-only and the owner can retry. A `cleanup_pending` retry stores only a same-tab marker keyed by the public snapshot ID—never an image key, URL, asset/owner/source ID, filename, token, alt text, or image data.
