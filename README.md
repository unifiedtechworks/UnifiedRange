...
Public-image reporting and moderator/admin hide/remove actions are planned but not implemented. The proposed Phase 2G design uses a snapshot-id-only report action, a backend-bound immutable image generation, safe moderator projections, separate report and image state machines, detach-first revocation, and no moderator access to private originals or image ledgers. See [Phase 2G Public Image Moderation Plan](docs/PUBLIC_IMAGE_PHASE_2G_MODERATION_PLAN.md).
