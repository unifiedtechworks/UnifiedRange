...
- Phase 2G.1 public-image moderation/report contract fields with client-nonwritable moderation state, protected exact-generation report binding, and resolver/processor blocked-state enforcement; no report or moderation UI yet
...
Phase 2G.1 reserves `public_image`, a reporter-nonwritable immutable asset-generation binding, and a separate owner-readable/client-nonwritable `clear | hidden | removed` image moderation state. New processing attempts initialize `clear` and cannot reuse a blocked generation; the public resolver returns generic unavailable for hidden, removed, or unknown state without returning moderation metadata. Existing rows with no state retain a temporary compatibility path until controlled backfill. Public-image reporting UI, safe moderator review projection/UI, moderator/admin hide/remove actions, notifications, and audit trails remain planned—not implemented—and moderators have no broad image-ledger or private-original access. See [Phase 2G Public Image Moderation Plan](docs/PUBLIC_IMAGE_PHASE_2G_MODERATION_PLAN.md).
