...
- Lambda processing and short-lived delivery for verified equipment-cover derivatives, rendered on saved Public Passport detail only
...
- Detail-only public rendering through a snapshot-ID-only resolver; Discover and public profile cards remain image-free
- Phase 2G public-image moderation/report guardrails, backend-bound detail-only reporting, metadata-only moderator review, and a separate admin/moderator current-image hide/remove action with detach-first delivery revocation
- A derived signed-in onboarding checklist on Dashboard and Profile; it adds no onboarding model

Public image rendering is limited to saved Public Passport detail pages. Phase 1 reserves a client-read-only workflow ledger and guarded public snapshot projection fields. Phase 2A registers successful private uploads as owner-only candidates, Phase 2B adds trusted private source verification, and Phase 2C adds the authenticated derivative processor. Phase 2D adds an owner-only Public Preview choice that defaults to **Publish without images** and can explicitly prepare one current verified JPEG/PNG Equipment Passport cover after a private preview, safety checklist, and alt text. Phase 2E.1 adds the `resolvePublicPassportImage` query, which accepts only a public snapshot id and returns either a 60-second non-cacheable derivative URL with safe alt text or one generic unavailable result. Phase 2E.2 calls it only from saved public detail. Phase 2F.1 adds the owner-authorized `removePublicPassportImage` backend primitive: it accepts only a snapshot id, detaches the guarded projection first, marks the matched ledger asset removed, and deletes only the canonical public derivative. Phase 2F.2 exposes that snapshot-id-only cleanup to the signed-in owner while keeping the sanitized text/setup snapshot published. Phase 2F.3 makes Unpublish derivative-aware. Phase 2F.4 adds owner-facing remove-first replacement: Public Preview detaches the old image, then reuses the existing verified-candidate consent and processor flow for a newly verified cover. Discover/profile cards remain image-free, browsers have no direct Storage access, target photos remain excluded, and private originals stay private. Atomic replacement remains unavailable. See [Phase 2D Public Image Consent UI](docs/PUBLIC_IMAGE_PHASE_2D_CONSENT_UI_PLAN.md), [Phase 2E Rendering Plan](docs/PUBLIC_IMAGE_PHASE_2E_RENDERING_PLAN.md), [Phase 2F Lifecycle Cleanup Plan](docs/PUBLIC_IMAGE_PHASE_2F_LIFECYCLE_CLEANUP_PLAN.md), [Phase 2F.1 Cleanup Testing](docs/PHASE_2F_1_LIFECYCLE_CLEANUP_TESTING.md), [Phase 2E.1 Delivery Resolver Testing](docs/PHASE_2E_1_DELIVERY_RESOLVER_TESTING.md), and [Public Image Backend Design](docs/PUBLIC_IMAGE_BACKEND_DESIGN.md).

Phase 2G.1 reserves `public_image`, a reporter-nonwritable immutable asset-generation binding, and a separate owner-readable/client-nonwritable `clear | hidden | removed` image moderation state. New processing attempts initialize `clear` and cannot reuse a blocked generation; the public resolver returns generic unavailable for hidden, removed, or unknown state without returning moderation metadata. Existing rows with no state retain a temporary compatibility path until controlled backfill. Phase 2G.2 adds **Report image** only after an eligible processed cover loads on saved Public Passport detail. Signed-in users submit an allow-listed reason and normalized bounded details against the public snapshot id; signed-out visitors receive a sign-in prompt. Phase 2G.3 gives group-authorized reviewers a distinct public-image report card and a link to the same sanitized public setup visible to visitors; it does not embed an image or read an image ledger. Phase 2G.4 adds a separate Cognito-group-authorized `moderatePublicPassportImage` mutation. The moderation UI sends only the public snapshot id, `hide | remove`, and an optional bounded owner-safe reason; the Lambda re-derives and conditionally validates the current attached `equipment_cover` asset and canonical derivative. **Hide** atomically detaches delivery and marks the asset hidden while preserving the processed object for later removal. **Remove** detaches first, marks the asset removed, deletes only the canonical public derivative, and returns `cleanup_pending` when safe cleanup needs a retry. Neither action changes `Report.status`, the sanitized public text/setup, or the private original.

Phase 2G.5 replaces the interim direct public-image report create with `createPublicImageReport`. The signed-in browser sends only the public snapshot id, allow-listed reason, and bounded optional details. Raw and normalized details are both capped at 500 characters. Before submission, the detail UI re-resolves delivery and stops if the rendered generation changed or disappeared. The Lambda then derives reporter identity and the exact current `ready + clear + equipment_cover` generation, revalidates the public snapshot/source/profile graph, and transactionally writes `Report.publicImageAssetId` plus `PublicImageAsset.lastReportAt`. Concurrent replacement, removal, unpublish, or visibility changes during backend validation/commit fail closed. Existing rows without a binding remain reviewable legacy/unbound reports, while malformed bindings are shown as unrecognized and never trusted. The Phase 2G.4 action is still current-snapshot scoped, so reviewers must inspect the current public setup before acting; exact-generation preview/action, cross-generation holds, audit, notifications, and lifecycle reconciliation remain unimplemented. Moderators have no broad image-ledger, private-record, or private-original access. See [Phase 2G Public Image Moderation Plan](docs/PUBLIC_IMAGE_PHASE_2G_MODERATION_PLAN.md).

Public API snapshot reads now use an explicit sanitized selection and cannot read `coverPhotoUrl`, `publicImageAssetId`, `publicImageKey`, or `publicImageAltText` through generated model operations. Public detail obtains only a short-lived processed derivative URL and safe alt text from `resolvePublicPassportImage`; no raw image key or generation id is returned publicly.

The moderation client explicitly uses Cognito user-pool authorization, while AppSync and the Lambda independently enforce `admin`/`moderator` membership. Hide preserves the processed derivative but stops new resolver delivery. If the owner later unpublishes that detached snapshot before moderator Remove, the action fails closed rather than guessing an old generation; the derivative remains non-deliverable and awaits protected reconciliation. Remove/finalization failures after detachment return bounded retry-safe cleanup state, never raw AWS details.

The hardened Phase 2F.2-2F.4 UI blocks publish, processing, removal, replacement, and unpublish operations from overlapping. Cleanup and unpublish responses are scoped to the route and signed-in owner; replacement also tracks the current private-cover source, so a late response cannot replace a newer Public Preview or continue after the source changes. Image-bearing Unpublish and remove-first replacement continue only after cleanup returns `removed`, `not_attached`, or detach-confirmed `cleanup_pending`; a failure leaves the text snapshot published and does not start the next step. If snapshot saving or replacement processing fails after detachment, the setup remains safely text-only and the owner can refresh and retry with a newly verified current cover. A `cleanup_pending` retry stores only a same-tab marker keyed by the public snapshot ID—never an image key, URL, asset/owner/source ID, filename, token, alt text, or image data.

The detail renderer fails closed. It validates the current snapshot-specific HTTPS/S3 path, bounded SigV4 lifetime and expiry, signed non-cacheable JPEG response overrides, safe alt text, and decoded dimensions; bounds resolver/image waits; ignores route-stale events; and never falls back to a private source or displays a raw resolver error.
...
Developers can test the deployed delivery resolver without a signed-in session by setting only `UNIFIEDRANGE_PUBLIC_PASSPORT_SNAPSHOT_ID` and running `npm run test:public-image-resolver`. The harness reads the normal API-key configuration from `amplify_outputs.json`, validates the 60-second public derivative URL and non-cacheable JPEG response internally, and prints no key, URL, alt text, ID, token, or image bytes. Follow [Phase 2E.1 Delivery Resolver Testing](docs/PHASE_2E_1_DELIVERY_RESOLVER_TESTING.md).

Developers can test the Phase 2F.1 backend directly, independently of the owner UI, by following [Phase 2F.1 Public Image Cleanup Testing](docs/PHASE_2F_1_LIFECYCLE_CLEANUP_TESTING.md) and running `npm run test:public-image-cleanup`. The harness requires only the public snapshot id, a short-lived matching owner ID token, and an exact confirmation phrase. It accepts and prints no key, URL, asset/owner/source id, filename, alt text, token, or image bytes.
...
9. Confirm success says the derivative is prepared for the saved Public Passport detail page while Discover/profile cards remain image-free.
10. Open Discover, the public passport detail, and the public profile signed in and signed out. Confirm only the eligible processed derivative renders on saved public detail and no private key or original is rendered or requested.
11. Confirm only the signed-in owner sees **Remove public image**, remove-first **Replace public image**, and derivative-aware **Unpublish**.
12. Confirm the removal dialog says the sanitized text/setup remains published and the private original remains private and unchanged.
13. Remove the public image and confirm the Public Preview refreshes to text-only state, the public detail no longer receives a derivative URL, and no raw key, ID, URL, or backend error appears.
14. Separately test Unpublish while a derivative is prepared. Confirm the image is detached before the sanitized text/setup is unpublished, the private original remains unchanged, and the public detail disappears.
15. Prepare another disposable derivative, upload and verify a new current equipment cover, then choose **Replace public image**. Confirm the old derivative becomes unavailable before the new consent flow starts, the sanitized text/setup stays public, the private original is unchanged, and the replacement requires the full safety checklist and new alt text.
16. Force replacement processing to fail after old-image cleanup. Confirm the setup stays public without an image, no old derivative is restored, and the owner can retry or cancel the replacement flow.
...
7. Open the public detail page and confirm sanitized setup fields display; an image appears only when an explicitly prepared derivative is currently eligible.
8. Sign out and confirm Discover and the public detail page still work, including eligible detail-only image delivery.
9. Test Unpublish with both a text-only snapshot and a prepared-derivative snapshot. Confirm the derivative-aware flow detaches public image delivery first, then removes the sanitized snapshot from Discover without changing the private Equipment Passport or private original.

Public pages remain based on sanitized text/setup snapshots. Only saved Public Passport detail may additionally request an explicitly prepared derivative through the short-lived delivery resolver; private S3 originals are never exposed publicly.
...
Comments, reactions, and reports are scoped to sanitized public passport snapshots. Public-image hide/remove is a separate group-authorized derivative action; it does not delete the report, public text/setup, private original, or account. This slice does not widen public image rendering or add feeds, follows, notifications, direct messages, marketplaces, or destructive account/content actions.
...
3. Submit a public passport/comment report and a **Report image** report from an eligible loaded public image.
...
8. Confirm the public-image report has a clear **Public image** target label, **Generation-bound report** badge for new reports (or **Legacy/unbound report** for old rows), muted public snapshot ID, optional shortened generation reference, reason/details, safe reporter identity, timestamps, and current status.
9. Confirm **Open public setup** opens only the sanitized public detail page in a new tab. No image is embedded in moderation and no image ledger, S3 key/path, owner/source ID, original filename, or private field is loaded or shown.
10. With a disposable current image, inspect that linked public setup immediately before confirming **Hide public image**. Confirm the detail resolver becomes unavailable while sanitized text/setup remains published and the private original remains unchanged.
11. Retry Hide and confirm the bounded already-hidden/no-op behavior. Then use **Remove public image** and confirm the retained public derivative is deleted or the UI reports retry-safe cleanup pending; a missing object is idempotent success.
12. Prepare another disposable current image and test **Remove public image** directly. Confirm delivery detaches before cleanup and no keys, URLs, image asset ids, owner ids, source ids, filenames, or raw errors appear in the UI or GraphQL input/result.
13. Confirm an optional moderation reason rejects URLs/storage paths, is limited to 240 characters, and contains no reporter identity or private data.
14. Change the report from `open` to `reviewed`, save, refresh, and confirm the status persists and the pending count decreases.
15. Test `dismissed` and `action_needed`, then return the report to the intended final workflow state.
16. Confirm report status changes never affect image availability, and hide/remove never silently changes report status.
17. Sign out and confirm the Moderation nav link disappears. Sign in as a normal non-moderator user and confirm `/moderation` denies access and direct mutation invocation is unauthorized.
18. Confirm the page does not show private passport data, private notes, private S3 keys, private images, purchase records, lot numbers, exact locations, or private profile fields.

Moderation access is enforced with Cognito user pool groups named `admin` and `moderator`. Those groups may update only `Report.status` on report records. Phase 2G.5 report creation is separately backend-controlled and generation-bound; the reporter cannot submit reporter/asset/key/source fields. A separate Phase 2G.4 mutation authorizes moderation groups to hide/remove only the current processed public derivative for a supplied public snapshot id; its Lambda derives all ledger and Storage identifiers and has no private-prefix access. Public-image reports use the same pending sorting and status workflow, with a public-detail link rather than an embedded preview. The action does not delete public text/setup, reports, accounts, or private originals. Only missing or `open` reports count as pending.
...
10. Discover: confirm filters work, cards remain image-free, and saved public detail shows only sanitized fields plus an eligible processed equipment-cover derivative—never private notes, private S3 keys/originals, target photos, maintenance/readiness records, ammo lot numbers, purchase details, exact locations, owner-private details, or image metadata.
...
The live backend boundary covers Cognito auth, owner-scoped records, private S3 images, owner-only private image candidates with trusted source verification, sanitized public snapshots, public identity snapshots, social actions, detail-only processed public-image delivery, generation-bound public-image reporting, owner image lifecycle controls, and group-gated report/public-image moderation. Broader public-image surfaces, exact-generation moderation actions, durable moderation audit/holds, account lifecycle execution, and secure username sign-in remain planned work.
