...
Public image rendering is limited to saved Public Passport detail pages. Phase 1 reserves a client-read-only workflow ledger and guarded public snapshot projection fields. Phase 2A registers successful private uploads as owner-only candidates, Phase 2B adds trusted private source verification, and Phase 2C adds the authenticated derivative processor. Phase 2D adds an owner-only Public Preview choice that defaults to **Publish without images** and can explicitly prepare one current verified JPEG/PNG Equipment Passport cover after a private preview, safety checklist, and alt text. Phase 2E.1 adds the `resolvePublicPassportImage` query, which accepts only a public snapshot id and returns either a 60-second non-cacheable derivative URL with safe alt text or one generic unavailable result. Phase 2E.2 calls it only from saved public detail. Phase 2F.1 adds the owner-authorized `removePublicPassportImage` backend primitive: it accepts only a snapshot id, detaches the guarded projection first, marks the matched ledger asset removed, and deletes only the canonical public derivative. The Phase 2F.2 Public Preview control now lets the signed-in owner invoke that snapshot-id-only cleanup while keeping the sanitized text/setup snapshot published and leaving the private original unchanged. Discover/profile cards remain image-free, browsers have no direct Storage access, target photos remain excluded, and private originals stay private. Replacement and derivative-aware unpublish remain unavailable. See [Phase 2D Public Image Consent UI](docs/PUBLIC_IMAGE_PHASE_2D_CONSENT_UI_PLAN.md), [Phase 2E Rendering Plan](docs/PUBLIC_IMAGE_PHASE_2E_RENDERING_PLAN.md), [Phase 2F Lifecycle Cleanup Plan](docs/PUBLIC_IMAGE_PHASE_2F_LIFECYCLE_CLEANUP_PLAN.md), [Phase 2F.1 Cleanup Testing](docs/PHASE_2F_1_LIFECYCLE_CLEANUP_TESTING.md), [Phase 2E.1 Delivery Resolver Testing](docs/PHASE_2E_1_DELIVERY_RESOLVER_TESTING.md), and [Public Image Backend Design](docs/PUBLIC_IMAGE_BACKEND_DESIGN.md).

The hardened Phase 2F.2 UI blocks publish, processing, removal, and unpublish operations from overlapping. Cleanup responses are scoped to the route and signed-in owner context, so a late response cannot replace a newer Public Preview state. A `cleanup_pending` retry stores only a same-tab marker keyed by the public snapshot ID—never an image key, URL, asset/owner/source ID, filename, token, alt text, or image data.
...
11. Confirm direct Unpublish and replacement remain disabled while a derivative is prepared, and that only the signed-in owner sees **Remove public image**.
...
14. Confirm text-only Unpublish is enabled after removal. Replacement and derivative-aware one-step unpublish remain separate future workflows.
